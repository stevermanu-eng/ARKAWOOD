(async () => {
  ...
  try {
    ...
    authSession = response.ok ? await response.json() : null;
  } catch (_) {
    authSession = null;
  }
  ...
  const STORAGE_KEY = `arkaWoodModerationApplicationV4:${discordUser.id}`;
  ...
  const categories = [
    ...
    },
    {
      ...
      questions: [
        { id: 'minecraftTime', number: 8, label: '¿Hace cuánto juegas Minecraft?', help: 'Solo para conocerte mejor y entender tu familiaridad con el juego.', type: 'text', placeholder: 'Ej. Juego desde hace 5 años...', required: true, min: 2, max: 160, icon: '⌛' },
        { id: 'dailyHours', number: 9, label: '¿Cuántas horas diarias puedes dedicar a labores de moderación?', help: 'Nos permite dimensionar tu aporte real y planificar turnos de cobertura.', type: 'number', placeholder: 'Ej. 2.5', required: true, minValue: 0.5, maxValue: 24, step: 0.5, icon: '◷' },
  ...
  const showToast = (message, kind = 'default', duration = 3200) => {
    if (!toast) return;
  ...
  const renderNavigation = () => {
    ...
    [...sidebar.querySelectorAll('[data-index]'), ...stepper.querySelectorAll('[data-index]')].forEach((button) => {
  ...
  const renderField = (question) => {
    ...
    if (question.type === 'select') {
      ...
      control = `<div class="form-control-wrap"><textarea id="${question.id}" name="${question.id}" rows="5" placeholder="${escapeHtml(question.placeholder)}" required>${escapeHtml(value)}</textarea><small class="char-count" data-for="${question.id}">${String(value).length}${question.min ? ` / ${question.min}+ mínimo` : ''}</small></div>`;
  ...
  const render = () => {
    ...
    questionRoot.querySelectorAll('input,select,textarea').forEach((field) => {
      field.addEventListener('input', () => updateFieldValue(field));
      field.addEventListener('change', () => updateFieldValue(field));
    });
  ...
  const renderReview = () => {
    ...
    reviewContent.querySelectorAll('[data-edit-category]').forEach((button) => {
      button.addEventListener('click', () => {
        state.reviewOpen = false;
        state.current = Number(button.dataset.editCategory);
        render();
        window.scrollTo({ top: 0, behavior: 'smooth' });
      });
    });
  ...
  const submitApplication = async () => {
    ...
    try {
      ...
      if (response.ok && result.ok) {
        ...
        showToast(result.message || 'Postulación enviada correctamente al equipo de ARKAWOOD.', 'success', 7000);
        reviewSubmit.innerHTML = 'POSTULACIÓN ENVIADA ✓';
        reviewSubmit.disabled = true;
        document.querySelectorAll('.application-review-category button, #applicationReviewBack, #applicationReviewSave').forEach((button) => { button.disabled = true; });
        const note = document.querySelector('.application-review-build-note');
        if (note) note.innerHTML = `<span>✓</span><p><b>Postulación recibida.</b> Discord confirmó el envío al canal interno de Moderación. ID de postulación: <code>${result.applicationId || 'generado por el servidor'}</code>. Guarda este identificador si necesitas consultar el proceso.</p>`;
        return;
      }
